...
impl GpuVerifier {
    ...
    pub async fn new() -> AmdResult<Self> {
        ...
        let adapter = instance
            ...
            .await
            .ok_or_else(|| AmdError::Other("未找到可用 GPU 适配器".into()))?;
        ...
        let (device, queue) = adapter
            ...
            .await
            .map_err(|e| AmdError::Other(format!("GPU 设备初始化失败: {e}").into()))?;
    ...
    pub async fn compute_blake3(&self, data: &[u8]) -> AmdResult<String> {
...
pub async fn auto_select_and_hash(data: &[u8]) -> AmdResult<String> {
