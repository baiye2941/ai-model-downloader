...
impl HttpClient {
    ...
    fn build_client(connect_secs: u64, read_secs: u64, enable_http2: bool) -> AmdResult<Self> {
        ...
        let client = builder
            .build()
            .map_err(|e| AmdError::Network(format!("创建 HTTP 客户端失败: {e}")))?;
        Ok(Self { client })
    }
...
fn validate_redirect_target(url: &reqwest::Url) -> AmdResult<()> {
    amd_core::validate_public_http_url(url)
}
...
fn classify_http_error(
    ...
    match code {
        429 | 503 => {
            ...
            AmdError::Throttled { retry_after_secs }
        }
        401 | 403 => AmdError::Forbidden { status: code },
        _ => AmdError::Protocol(format!("HTTP {status}")),
    }
}
...
impl Protocol for HttpClient {
    fn probe(
        ...
        Box::pin(async move {
            ...
            if !status.is_success() {
                warn!(url = %amd_core::redact_url_for_log(&url), status = %status, "HEAD 请求返回非成功状态码");
            ...
            info!(
                url = %amd_core::redact_url_for_log(&url),
                file_size = ?file_size,
    ...
    fn download_range(
        ...
        Box::pin(async move {
            ...
            debug!(url = %amd_core::redact_url_for_log(&url), start, end, "HTTP Range 请求开始");
            let response = client
                .get(&url)
                .header("Range", &range)
                .send()
            ...
            if status == reqwest::StatusCode::OK {
                warn!(url = %amd_core::redact_url_for_log(&url), "服务器忽略 Range 头,返回 HTTP 200");
                return Err(AmdError::Protocol(
                    "服务器忽略 Range 头,返回 HTTP 200(不支持分片下载)".into(),
                ));
            }
            if status != reqwest::StatusCode::PARTIAL_CONTENT {
                warn!(url = %amd_core::redact_url_for_log(&url), status = %status, "Range 请求返回非预期状态码");
                return Err(classify_http_error(status, response.headers()));
            }
            ...
            let bytes = response
                .bytes()
                .await
                .map_err(|e| AmdError::Network(format!("读取响应体失败: {e}")))?;
    ...
    fn download_range_stream(
        ...
        Box::pin(async move {
            ...
            amd_core::validate_public_http_url(&parsed_url)?;
            let range = format!("bytes={start}-{end}");
            debug!(url = %amd_core::redact_url_for_log(&url), start, end, "HTTP 流式 Range 请求开始");
            let response = client
                .get(&url)
                .header("Range", range)
                .send()
            ...
            if status != reqwest::StatusCode::PARTIAL_CONTENT {
                warn!(url = %amd_core::redact_url_for_log(&url), status = %status, "流式 Range 请求返回非预期状态码");
                return Err(classify_http_error(status, response.headers()));
            }

            info!(url = %amd_core::redact_url_for_log(&url), start, end, "HTTP 流式 Range 响应头已接收,开始流式传输");

            // 使用 bytes_stream() 获取真正的数据流,
            // 调用方通过 StreamExt::next() 逐块消费,峰值内存仅包含单个 chunk
            let stream = response.bytes_stream().map(|result| {
                result.map_err(|e| AmdError::Network(format!("读取响应流数据失败: {e}")))
            });
    ...
    fn download_full_stream(
        ...
        Box::pin(async move {
            let parsed_url = reqwest::Url::parse(&url)?;
            amd_core::validate_public_http_url(&parsed_url)?;
            debug!(url = %amd_core::redact_url_for_log(&url), "HTTP 整块流式请求开始");
            let response = client
                .get(&url)
                .send()
                .await
            ...
            info!(url = %amd_core::redact_url_for_log(&url), "HTTP 整块流式响应头已接收,开始流式传输");
