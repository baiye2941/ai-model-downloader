...
pub struct TaskSnapshot {
    ...
    pub status: amd_core::DownloadState,
...
fn parse_legacy_status(status: &str) -> amd_core::DownloadState {
    match status {
        "pending" => amd_core::DownloadState::Pending,
        "downloading" => amd_core::DownloadState::Downloading,
        "paused" => amd_core::DownloadState::Paused,
        "completed" => amd_core::DownloadState::Completed,
        "failed" => amd_core::DownloadState::Failed,
        "cancelled" => amd_core::DownloadState::Cancelled,
        "verifying" => amd_core::DownloadState::Verifying,
        _ => amd_core::DownloadState::Failed,
    }
}
...
impl RecoveryManager {
    ...
    pub fn recover_pending_snapshots(&self) -> std::io::Result<Vec<TaskSnapshot>> {
        ...
        for snapshot in self.load_all_task_snapshots()? {
            if matches!(
                snapshot.status,
                amd_core::DownloadState::Downloading | amd_core::DownloadState::Paused
            ) {
...
mod tests {
    ...
    fn make_snapshot(id: &str, status: amd_core::DownloadState) -> TaskSnapshot {
        TaskSnapshot {
            id: id.to_string(),
    ...
    fn snapshot_save_and_load() {
        ...
        let store = KvStore::open(tmp.path()).unwrap();
        let mgr = RecoveryManager::new(store);
        let snap = make_snapshot("s1", amd_core::DownloadState::Downloading);
    ...
    fn snapshot_load_all() {
        ...
        let store = KvStore::open(tmp.path()).unwrap();
        let mgr = RecoveryManager::new(store);
        mgr.save_task_snapshot(&make_snapshot("a", amd_core::DownloadState::Downloading))
            .unwrap();
        mgr.save_task_snapshot(&make_snapshot("b", amd_core::DownloadState::Completed))
            .unwrap();
        mgr.save_task_snapshot(&make_snapshot("c", amd_core::DownloadState::Paused))
            .unwrap();
    ...
    fn snapshot_recover_pending() {
        ...
        let store = KvStore::open(tmp.path()).unwrap();
        let mgr = RecoveryManager::new(store);
        mgr.save_task_snapshot(&make_snapshot("p1", amd_core::DownloadState::Downloading))
            .unwrap();
        mgr.save_task_snapshot(&make_snapshot("p2", amd_core::DownloadState::Completed))
            .unwrap();
        mgr.save_task_snapshot(&make_snapshot("p3", amd_core::DownloadState::Paused))
            .unwrap();
        mgr.save_task_snapshot(&make_snapshot("p4", amd_core::DownloadState::Failed))
            .unwrap();
    ...
    fn snapshot_to_record_conversion() {
        let snap = make_snapshot("conv", amd_core::DownloadState::Downloading);
    ...
    fn snapshot_empty_fragments() {
        ...
        let store = KvStore::open(tmp.path()).unwrap();
        let mgr = RecoveryManager::new(store);
        let snap = make_snapshot("empty", amd_core::DownloadState::Downloading);
        mgr.save_task_snapshot(&snap).unwrap();
    ...
    fn snapshot_recovers_legacy_task_record_json() {
        ...
        assert_eq!(pending[0].status, amd_core::DownloadState::Paused);
    }
    ...
    fn test_task_snapshot_serializes_typed_status_and_metadata() {
        let snapshot = TaskSnapshot {
            ...
            total_fragments: 4,
            fragment_size: 256,
            status: amd_core::DownloadState::Paused,
        ...
        assert_eq!(loaded.status, amd_core::DownloadState::Paused);
