...
use crate::error::AmdResult;
use crate::types::{FileMetadata, FragmentInfo, TaskId};

/// 字节流类型别名
///
/// 用于 `download_range_stream` 的返回值,逐块产出 `AmdResult<Bytes>`。
/// 调用方应使用 `StreamExt::next()` 逐块消费,避免将整个响应缓冲到内存。
pub type ByteStream = Pin<Box<dyn Stream<Item = AmdResult<Bytes>> + Send>>;
...
pub trait Protocol: Send + Sync {
    ...
    fn probe(&self, url: &str) -> Pin<Box<dyn Future<Output = AmdResult<FileMetadata>> + Send>>;
    ...
    ) -> Pin<Box<dyn Future<Output = AmdResult<Bytes>> + Send>>;
    ...
    ) -> Pin<Box<dyn Future<Output = AmdResult<ByteStream>> + Send>>;

    /// 下载整个文件(不支持 Range 时使用)
    fn download_full(&self, url: &str) -> Pin<Box<dyn Future<Output = AmdResult<Bytes>> + Send>>;
...
pub trait Verifier: Send + Sync {
    /// 计算数据的哈希值
    fn compute_hash(&self, data: &[u8]) -> AmdResult<String>;

    /// 校验数据是否匹配预期哈希
    fn verify(&self, data: &[u8], expected_hash: &str) -> AmdResult<()> {
        let actual = self.compute_hash(data)?;
        if actual == expected_hash {
            Ok(())
        } else {
            Err(crate::error::AmdError::ChecksumMismatch {
...
pub trait DownloadTask: Send + Sync {
    ...
    /// 获取文件元数据
    fn metadata(&self) -> AmdResult<&FileMetadata>;
...
pub trait FragmentDownloader: Send + Sync {
    ...
    ) -> impl std::future::Future<Output = AmdResult<Bytes>> + Send;

    /// 取消分片下载
    fn cancel(&self, task_id: TaskId, fragment_index: u32) -> AmdResult<()>;
}
