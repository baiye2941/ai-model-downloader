...
pub mod harness {
    ...
    use crate::error::{AmdError, AmdResult};
    ...
    impl MockProtocol {
        ...
        pub fn failing(error: AmdError) -> Self {
            Self {
                metadata: None,
    ...
    impl Protocol for MockProtocol {
        ...
        fn download_range(
            ...
            Box::pin(async move {
                ...
                data.get(&(start, end))
                    .cloned()
                    .ok_or_else(|| AmdError::Network(format!("未找到范围数据: {start}-{end}")))
            })
        }
        ...
        ) -> Pin<Box<dyn std::future::Future<Output = AmdResult<crate::traits::ByteStream>> + Send>>
        {
            let this = self.clone();
        ...
        fn download_full(
            ...
            Box::pin(async move {
                this.default_data
                    .clone()
                    .ok_or_else(|| AmdError::Protocol("不支持全量下载".into()))
            })
        }
    }
...
mod tests {
    ...
    async fn test_mock_protocol_failing() {
        let protocol = MockProtocol::failing(AmdError::Network("连接超时".into()));
