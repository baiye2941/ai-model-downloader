...
use crate::{QfError, QfResult};

pub fn validate_public_http_url(url: &Url) -> QfResult<()> {
    match url.scheme() {
        "http" | "https" => {}
        scheme => return Err(QfError::Config(format!("不支持的协议: {scheme}"))),
    }

    if !url.username().is_empty() || url.password().is_some() {
        return Err(QfError::Config("URL 中不允许包含用户名或密码".into()));
    }
    ...
    if normalized_host.eq_ignore_ascii_case("localhost") {
        return Err(QfError::Config("不允许访问 localhost".into()));
    }
...
pub fn reject_forbidden_ip(ip: IpAddr) -> QfResult<()> {
    match ip {
...
fn reject_forbidden_ipv4(ip: Ipv4Addr) -> QfResult<()> {
    if ip.is_loopback()
        || ip.is_private()
        || ip.is_link_local()
        || ip.is_unspecified()
    ...
    {
        return Err(QfError::Config(format!("不允许访问受限 IPv4 地址: {ip}")));
    }
    Ok(())
}

fn reject_forbidden_ipv6(ip: Ipv6Addr) -> QfResult<()> {
    ...
    if ip.is_loopback() || ip.is_unspecified() || ip.is_multicast() || unique_local || link_local {
        return Err(QfError::Config(format!("不允许访问受限 IPv6 地址: {ip}")));
    }
    Ok(())
}
