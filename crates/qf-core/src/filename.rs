...
pub fn validate_save_path(
    ...
    let parent = final_path
        .parent()
        .ok_or_else(|| crate::QfError::Config("无效的文件路径: 无父目录".into()))?;

    if !parent.exists() {
        std::fs::create_dir_all(parent).map_err(crate::QfError::Io)?;
    }

    let canonical_parent = parent
        .canonicalize()
        .map_err(|e| crate::QfError::Config(format!("无法解析父目录: {e}")))?;
    ...
    let file_name = final_path
        .file_name()
        .ok_or_else(|| crate::QfError::Config("无效的文件路径: 无文件名".into()))?;
