...
description: 执行包含独立任务的实现计划。支持两种模式：子代理模式（推荐，用于 Claude Code 等支持子代理的环境）和直接执行模式（备选，用于无子代理支持的环境）。
---
...
digraph when_to_use {
    ...
    "子代理可用?" [shape=diamond];
    "子代理模式" [shape=box, style=filled, fillcolor=lightgreen];
    "直接执行模式" [shape=box];
    "手动执行或先头脑风暴" [shape=box];

    "有实现计划?" -> "任务基本独立?" [label="是"];
    "有实现计划?" -> "手动执行或先头脑风暴" [label="否"];
    "任务基本独立?" -> "子代理可用?" [label="是"];
    "任务基本独立?" -> "手动执行或先头脑风暴" [label="否 - 紧密耦合"];
    "子代理可用?" -> "子代理模式" [label="是（推荐）"];
    "子代理可用?" -> "直接执行模式" [label="否"];
}
```
...
**适用环境：** Claude Code、Codex 等支持子代理的平台
...
2. 逐个执行任务，每任务完成后验证
3. 每完成 3 个任务后批量审查
4. 所有任务完成后使用 finishing-a-development-branch

**注意事项：**
- 先批判性审查计划（检查依赖、验证条件、环境假设）
- 严格按照计划步骤执行
- 遇到阻塞时停下来，不要猜测
